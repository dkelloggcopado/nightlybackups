trigger promoteduserstory on copado__Promoted_User_Story__c (after insert) {
    ...
    For(copado__Promoted_User_Story__c pro : Trigger.new )
    {
        ...
        PromotionList = [SELECT copado__Promotion__c FROM copado__Promoted_User_Story__c where copado__User_Story__c =:userStoryID and copado__Promoted_User_Story__c.copado__Promotion__r.copado__Back_Promotion__c = true and copado__Promoted_User_Story__c.copado__Promotion__r.copado__Status__c = 'Completed'ORDER BY CreatedDate DESC];
        ...
        if(PromotionList.size() > 0 && (PromotionList[0].copado__User_Story__r.copado__Latest_Commit_Date__c > PromotionList[0].copado__Promotion__r.CreatedDate)){
            For(copado__Promoted_User_Story__c objpromoteduserstory: PromotionList ){
